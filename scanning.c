...
void pointToSmallestObject(double *distances, int length, int intermittentAngle){
    int i;
    ...
    for(i = 1; i < length; i ++){
        ...
        if(tracking){
            objArr[objNum].width += intermittentAngle;
        }
...
void scanIntermitently(int intermitentAngle, int angleDesired, cyBOT_Scan_t* scanner){
   ...
   // float previousAverage = 0.0;
    ...
    object objArr[10];
    memset(objArr, 0, sizeof(objArr)); //Setting everything in struct array to 0

    cyBOT_Scan(0, scanner);
    ...
    while(totalAngle < angleDesired){
       ...
       // average /= 3;
        ...
        if((abs(doublePreviousInf - currentInf) > differenceEpsilon) && tracking == 0 && stoppedTrackingLastTime == 0){
            ...
            if(objArr[objNum].sampleCount > 0){
                objArr[objNum].distance = objArr[objNum].totalDistance / objArr[objNum].sampleCount;
            }
            ...
            sprintf(message2, " Distance %f, radial width %d, linear Width %f\n\r",
                    objArr[objNum].distance,
                    objArr[objNum].width,
                    objArr[objNum].linearWidth);
            ...
            if(tracking){
                objArr[objNum].totalDistance += pingVal;
                objArr[objNum].sampleCount++;
            }
            if(stoppedTrackingLastTime){
